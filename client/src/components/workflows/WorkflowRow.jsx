...
const statusStyles = {
  ...
  Inactive: "bg-zinc-800/70 text-zinc-500 border-zinc-700/50",
};

const WorkflowRow = ({ workflow, onMenuClick }) => {
  const TriggerIcon = triggerIcons[workflow.trigger] || FiLink;
      ...
      <div>
        <span
          className={`inline-flex items-center rounded-full border px-2 py-1 text-[11px] font-medium ${
            statusStyles[workflow.status] || statusStyles.Inactive
          }`}
        >
